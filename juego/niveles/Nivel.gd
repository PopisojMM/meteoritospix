...
func crear_sector_enemigos(num_enemigos: int) -> void:
	for i in range(num_enemigos):
		var _new_interceptor:EnemigoInterceptor = enemigo_interceptor.instance()
		var spawn_pos: Vector2 = crear_posicion_aleatoria(1000.0, 800.0)
		_new_interceptor.global_position = player.global_position + spawn_pos
		contenedor_enemigos.add_child(_new_interceptor)
...
func destruir_nivel() -> void:
	crear_explosion(player.global_position, 5, 1.0, Vector2(300.0, 200.0))
	player.destruir()
...
func _on_nave_destruida(nave: Player, posicion: Vector2) -> void:
	if nave is Player:
		transicion_camaras(
			posicion,
